...
static numbers: [&str; 18] = ["1","2","3","4","5","6","7","8","9","one","two","three","four","five","six","seven","eight","nine"];
...
fn find_number(word: &str, is_first: bool) -> &str {
    let mut first_found_values = vec![];
    let mut return_value = "";

    for number in numbers {
        let positions: Vec<_> = word.match_indices(number).into_iter().collect();
        for position in positions {
            first_found_values.push(Word { value: position.1, index: position.0 as i32 })
        }
    }

    first_found_values.sort_by(|a,b| a.index.cmp(&b.index));

    if is_first {
        return_value = first_found_values[0].value;
    } else {
        return_value = first_found_values[first_found_values.len() - 1].value;
    }

    if return_value.len() > 1 {
        return_value = convert_to_number(return_value);
    }
...
fn main() {
    let test_coords = "one2mgnphzcxeight45rmnffneight";

    let coordinates = "5ffour295
    ...
    let mut result = 0;
    let rows = coordinates.split("\n");

    for row in rows {
        let first = find_number(row, true);
        let second = find_number(row, false);

        let value = first.to_owned() + second;
