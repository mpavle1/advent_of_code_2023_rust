...
const bag_values: [i32; 3] = [12, 13, 14]; //r,g,b


fn max_value(val: &str) -> i32 {
    match val {
        "red" => 12,
        "green" => 13,
        "blue" => 14,
...
fn main() {
    ...
    for game in games {
        ...
        let combinations = split_values[1].split(";").collect::<Vec<_>>();

        let mut min_values = [1,1,1];

        for combination in combinations {
            let colors = combination.split(",").collect::<Vec<_>>();
           
            for color in colors {
                let count_and_color = color.trim().split(" ").collect::<Vec<_>>();

                let count = count_and_color[0].parse::<i32>().unwrap();
                let seg_color = count_and_color[1];

                match seg_color {
                    "red" => {
                        min_values[0] = cmp::max(min_values[0], count);
                    },
                    "green" => {
                        min_values[1] = cmp::max(min_values[1], count);
                    },
                    "blue" => {
                        min_values[2] = cmp::max(min_values[2], count);
                    },
                    &_ => todo!()
                }
            }
        ...
        sum +=  min_values[0] *  min_values[1] *  min_values[2];
    }
